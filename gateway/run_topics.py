...
class GatewayTopicThreadsMixin:
    ...
    def _telegram_cooldown_elapsed(self, source: SessionSource, attr: str, cooldown_s: float) -> bool:
        ...
        # `None`, not 0.0, is the "never sent" sentinel: time.monotonic() is measured from an
        # arbitrary origin (system boot on Linux), so on a freshly booted machine `now` itself is
        # inside the cooldown window and a 0.0 default would swallow the very first reminder.
        last = stamps.get(key)
