...
class GatewayNotificationsMixin:
    ...
    async def _self_post_api_server(self, adapter, synth_text: str, raw_sid: str, evt: dict) -> bool:
        ...
        if evt.get("type") == "async_delegation":
            ...
            # Run-scoped Nolgia credential of the turn that SPAWNED this work (async-delegation
            # completions only, in-memory events only — see tools.async_delegation
            # ._attach_origin_nolgia_token) and the multiplex profile that served it
            # (HERMES_SESSION_PROFILE, captured at dispatch/registration). Handed to the api_server
            # self-post so the continuation turn attributes its CLI work to that run — not the
            # session's most recent one — and re-enters through /p/<profile>/... (the unprefixed
            # route would resume under the DEFAULT profile, which has neither the session's history
            # nor its retained credential) (NOL-413).
            deliver = lambda: deliver_wake(  # noqa: E731
                adapter, text=synth_text, session_id=raw_sid,
                nolgia_token=str(evt.get("origin_nolgia_token") or ""),
    ...
    def _build_process_completion_event(watcher: dict, session, session_id: str) -> dict:
        ...
        return {
            ...
            **{k: watcher.get(k, "") for k in _WATCHER_ROUTE_FIELDS},
            ...
            # Profile that owns the spawning session — an api_server wake self-post has to re-enter
            # through that profile's /p/<profile>/ route (NOL-413).
            "origin_profile": str(watcher.get("profile") or "").strip(),
