...
class OpenAICompatRoutesMixin:
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        from gateway.platforms.api_server import (
            ...
            _openai_error, _platform_asset_resolver_enabled, _redact_api_error_text,
            _request_nolgia_token, _resolve_media_tags_to_platform_assets, _resolve_media_to_data_urls)
        ...
        # Run-scoped Nolgia platform credential. Pod-internal continuation turns reach this endpoint
        # through gateway.wake's self-post and carry the credential of the ORIGINATING run whose
        # detached delegation just finished, so the continuation's CLI work attributes to THAT turn
        # even when the session has started newer turns since (NOL-413). Absent one, _run_agent
        # falls back to the session's retained run token.
        ...
        if _platform_asset_resolver_enabled():
            # Platform-relay mode (this endpoint is the relay's legacy fallback and the wake
            # self-post target): upload MEDIA:-tagged files as platform assets instead of inlining
            # 5MB base64 blobs into the transcript. This hop runs on the request task AFTER
            # _run_agent cleared the executor thread's session binding, so re-bind the run credential
            # (the request's own when a wake self-post carried the originating run's token, else the
            # session's retained one — NOL-413) for these uploads to attribute to the causing turn.
            # Look up by the REQUEST session id first — the key the /v1/runs submission retained (a
            # mid-turn compression rotation changes result["session_id"] but not the retained key) —
            # under THIS request's profile scope, which is part of the retention identity.
            from gateway.session_context import reset_session_nolgia_token, set_session_nolgia_token
            retained_token = nolgia_token or self._retained_session_run_token(
                str(session_id or result.get("session_id") or ""), profile=str(_api_request_profile.get() or ""))
            reset = set_session_nolgia_token(retained_token) if retained_token else None
            try:
