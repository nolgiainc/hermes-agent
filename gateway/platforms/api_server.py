...
# Wire spelling of the default profile in a /p/<profile>/ prefix. ``/v1/...`` and
# ``/p/default/v1/...`` are aliases for the same runtime, so anything that uses the profile as an
# identity (see _session_run_token_key) must treat this name and ``""`` as one scope.
...
class ResponseStore:
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        # Durable terminal run statuses (NOL-93). A /v1/runs turn can outlive its supervisor: the
        # Nolgia platform budget-fails a turn while the pod keeps executing and completes minutes
        # later. The in-memory _run_statuses copy dies with the process (and with its 1h TTL), so a
        # late GET /v1/runs/{id} salvage probe would 404 and the caller's only recourse is
        # re-running the whole turn — double side effects, double credit spend. Terminal statuses
        # (completed/failed/cancelled) are therefore mirrored here. In-flight statuses are never
        # mirrored into THIS table (a durable "running" row would lie about lost work forever); the
        # run_journal below is what makes an in-flight run's existence survive a restart, as an
        # honest failure. ``owner_scope`` is the authenticated request scope that submitted the run
        # (the same opaque sha256 the run idempotency store keys on — never a credential): the
        # fail-closed run-ownership boundary (#93689) must still hold for a status served from disk
        # after a restart, so a row is only served to the scope that owns it.
        self._conn.execute(
        ...
        # Restart-durable run journal (NOL-423). One row per /v1/runs submission, inserted before
        # the run is visible as queued, updated on low-frequency status transitions and throttled
        # heartbeats, and settled (settled=1) in the SAME transaction as the terminal status write.
        # Its consumer is the NEXT boot: reconcile_prior_boot_runs() turns rows a dead process left
        # unsettled into durable `failed` statuses with error_code="gateway_restart" + provenance,
        # so a replacement pod answers a status poll honestly instead of 404ing and the platform's
        # capability-gated auto-resubmit (NOL-397 part 1) can act on a machine-matchable signal.
        # Rows carry identity and provenance only — hashes, sizes, bounded redacted tails — never
        # full payloads (the platform durably owns submission content) and never credentials.
        # Resume-from-journal is deliberately out of scope: honest-failure-with-provenance first.
        self._conn.execute(
        ...
        # owner_scope was added after the first fork schema shipped to the fleet; migrate in place.
        for table in ("run_statuses", "run_journal"):
            columns = {str(row[1]) for row in self._conn.execute(f"PRAGMA table_info({table})")}
            if "owner_scope" not in columns:
                self._conn.execute(f"ALTER TABLE {table} ADD COLUMN owner_scope TEXT NOT NULL DEFAULT ''")
        self._conn.commit()
        # _set_run_status can be reached from the run's executor thread (a late tool_progress
        # event) as well as the event loop; sqlite3 shares one connection here, so run-status
        # reads/writes take this lock.
    ...
    def db_path(self) -> Optional[str]:
        ...
        None means every row here dies with the process: __init__ falls back to ``:memory:`` when
        the configured path cannot be opened. Callers that promise durability (the run journal's
        restart contract) MUST gate that promise on this being set.
        """
    ...
    # Retention comfortably outlives every platform-side salvage window (the Nolgia relay's
    # reclaim/sweep chain is measured in minutes; ops post-mortems in hours/days) while keeping
    # the table bounded.
    RUN_STATUS_RETENTION_SECONDS = 48 * 3600
    MAX_STORED_RUN_STATUSES = 500

    def put_run_status(self, run_id: str, data: Dict[str, Any], owner_scope: Optional[str] = None) -> None:
        """Persist a terminal run status (INSERT OR REPLACE) and prune."""
        with self._run_status_lock:
            self._put_run_status_locked(run_id, data, time.time(), owner_scope)
            self._conn.commit()

    def _stored_owner_scope_locked(self, run_id: str) -> str:
        """Owner scope already recorded for *run_id* (terminal row, else journal row), or ""."""
        for table in ("run_statuses", "run_journal"):
            row = self._conn.execute(
                f"SELECT owner_scope FROM {table} WHERE run_id = ?", (run_id,)).fetchone()
            if row is not None and row[0]:
                return str(row[0])
        return ""

    def _put_run_status_locked(
        self, run_id: str, data: Dict[str, Any], now: float, owner_scope: Optional[str] = None,
    ) -> None:
        """Write + prune a terminal status. Caller holds the lock and commits. ``owner_scope=None``
        keeps whatever scope the run already recorded (journal row or an earlier terminal write)."""
        if owner_scope is None:
            owner_scope = self._stored_owner_scope_locked(run_id)
        self._conn.execute(
            "INSERT OR REPLACE INTO run_statuses (run_id, data, updated_at, owner_scope) VALUES (?, ?, ?, ?)",
            (run_id, json.dumps(data, default=str), now, str(owner_scope or "")),
        )
        self._conn.execute(
            "DELETE FROM run_statuses WHERE updated_at < ?", (now - self.RUN_STATUS_RETENTION_SECONDS,))
    ...
    def get_run_status(self, run_id: str) -> Optional[Dict[str, Any]]:
        ...
        Retention is enforced here as well as in put_run_status: the write path only prunes when
        a NEWER terminal status lands, so on a quiet gateway an expired row would otherwise stay
        servable forever.
        """
        with self._run_status_lock:
            row = self._conn.execute(
                "SELECT data, updated_at FROM run_statuses WHERE run_id = ?", (run_id,)).fetchone()
            if row is not None and (time.time() - float(row[1] or 0) > self.RUN_STATUS_RETENTION_SECONDS):
        ...
        try:
            ...
            logger.warning("Corrupted JSON in run status store for run_id=%s, evicting entry", run_id)
            with self._run_status_lock:
                self._conn.execute("DELETE FROM run_statuses WHERE run_id = ?", (run_id,))
                self._conn.commit()
            return None

    def run_owner_scope(self, run_id: str) -> str:
        """Authenticated scope that submitted *run_id* (terminal row, else journal row), or "".

        Lets the fail-closed run-ownership check (#93689) admit a status served from disk after a
        restart ONLY to the scope that submitted it; an unstamped row stays unanswerable.
        """
        with self._run_status_lock:
            return self._stored_owner_scope_locked(run_id)

    # Restart-durable run journal (NOL-423) ----------------------------------------------

    def journal_run_submitted(
        self, run_id: str, boot_uuid: str, data: Dict[str, Any], owner_scope: str = "",
    ) -> None:
        ...
        with self._run_status_lock:
            self._conn.execute(
                """INSERT OR REPLACE INTO run_journal
                   (run_id, boot_uuid, status, settled, created_at, updated_at, data, owner_scope)
                   VALUES (?, ?, ?, 0, ?, ?, ?, ?)""",
                (
                    run_id, boot_uuid, str(data.get("status", "queued")),
                    float(data.get("created_at") or now), now, json.dumps(data, default=str),
                    str(owner_scope or ""),
                ),
            )
            # Retention and capacity BOTH apply to settled rows only. An unsettled row is the only
            # record the next boot has of an in-flight run, and a run can legitimately go quiet for
            # longer than retention (an approval waiting on a human, a long external operation), so
            # age alone must never delete it — otherwise the next boot 404s a run this gateway still
            # claims to journal. Capacity is likewise a bound on settled history: exceeding it is
            # preferable to erasing live runs.
            self._conn.execute(
                "DELETE FROM run_journal WHERE settled = 1 AND updated_at < ?",
                (now - self.RUN_STATUS_RETENTION_SECONDS,),
            )
            count = self._conn.execute("SELECT COUNT(*) FROM run_journal").fetchone()[0]
            if count > self.MAX_STORED_RUN_STATUSES:
                self._conn.execute(
    ...
    def journal_run_update(self, run_id: str, status: Optional[str] = None, **fields: Any) -> None:
        """Merge a status transition / heartbeat into a live journal row.

        No-ops when the run was never journaled (statuses set outside the /v1/runs submission
        path) or its row is already settled (a late tool event landing after the terminal settle
        must not reopen it).
        """
        now = time.time()
        with self._run_status_lock:
            row = self._conn.execute(
                "SELECT status, data, settled FROM run_journal WHERE run_id = ?", (run_id,)).fetchone()
    ...
    def settle_run_status(
        self, run_id: str, data: Dict[str, Any], owner_scope: Optional[str] = None,
    ) -> None:
        """Persist a terminal status AND settle the journal row atomically.

        One transaction on purpose: reconciliation trusts ``settled`` to mean "a durable terminal
        status exists", so the pair must be indivisible — a terminal write without the settle
        would let the next boot double-mark a finished run as gateway_restart-failed, and a settle
        without the terminal write would silently drop the outcome. Idempotent: replaying the
        settle rewrites the same rows.
        """
        now = time.time()
        with self._run_status_lock:
            self._put_run_status_locked(run_id, data, now, owner_scope)
            self._conn.execute(
    ...
    def reconcile_prior_boot_runs(
        ...
        Runs once at adapter startup, before any request is served. Rows whose ``boot_uuid``
        differs from the current boot and that never settled are, by construction, runs whose
        in-flight work this process is not executing — the prior process died with them. Each
        becomes a durable ``failed`` status with the stable, machine-matchable
        ``error_code: "gateway_restart"`` plus provenance (original created_at, last journaled
        status/event/heartbeat, prior boot/pod identity, bounded redacted output tail), so a late
        poll answers honestly instead of 404ing and the platform can treat the failure as
        auto-resubmit-eligible.

        Liveness is the boot-identity comparison, never heartbeat staleness: a paused/slow process
        is not dead, and same-boot rows are never touched. If a durable terminal status already
        exists for a row (completed-during-shutdown on an image that wrote statuses without
        settling), the real outcome wins and the row is only settled. Returns the failed statuses
        written.
        """
        now = time.time()
        ...
        with self._run_status_lock:
            ...
            for run_id, boot_uuid, last_status, created_at, journaled_at, raw in rows:
                ...
                existing = self._conn.execute(
                    "SELECT 1 FROM run_statuses WHERE run_id = ?", (run_id,)).fetchone()
                if existing is None:
                    provenance: Dict[str, Any] = {
                        "boot_uuid": boot_uuid, "last_status": last_status,
                        "last_journal_write_at": journaled_at, "reconciled_at": now}
                    ...
                    status_payload: Dict[str, Any] = {
                        "object": "hermes.run", "run_id": run_id, "status": "failed",
                        "error": (
                            "gateway restarted while the run was in flight; the in-flight work "
                            "was lost and the submission can be safely retried"),
                        "error_code": "gateway_restart", "created_at": created_at, "updated_at": now,
                        "last_status": last_status, "restart_provenance": provenance}
                    for key in ("session_id", "model", "last_event"):
                        if data.get(key) is not None:
                            status_payload[key] = data[key]
                    # owner_scope=None: the terminal row inherits the journal row's scope.
                    self._put_run_status_locked(run_id, status_payload, now)
                    reconciled.append(status_payload)
                self._conn.execute(
                    "UPDATE run_journal SET settled = 1, status = ?, updated_at = ? WHERE run_id = ?",
                    ("failed" if existing is None else str(last_status), now, run_id),
                )
            if rows:
                self._conn.commit()
        return reconciled
    ...
    def get_journal_row(self, run_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            data = {}
        return {
            "boot_uuid": row[0], "status": row[1], "settled": bool(row[2]),
            "created_at": row[3], "updated_at": row[4], "data": data}
...
# ---------------------------------------------------------------------------
# Reconciliation converts every unsettled prior-boot journal row into a durable gateway_restart
# failure, which the platform treats as auto-resubmit-eligible. Doing that while the boot that
# owns those rows is still executing them duplicates tool side effects and credit spend, so a
# boot may only reconcile after proving no live gateway owns the store.
#
# The machine-local scoped lock (gateway.status) provides the cross-process half of that proof,
# but it is re-entrant per PID and therefore cannot separate two adapters inside ONE process
# (multiplexed startup, reconnect churn constructing a replacement adapter before the old one is
# collected) that share a response_store.db. This registry is the in-process half: db_path ->
# weakref to the owning adapter. Ownership passes on only when the current owner is gone or no
# longer connected.
...
def _platform_asset_resolver_enabled() -> bool:
    ...
    See ``gateway.platforms.nolgia_assets.resolver_enabled``: the platform deployment injects
    ``NOLGIA_API_URL`` + ``NOLGIA_TOKEN`` into the gateway container, so their presence is the
    platform-mode signal. Without them every egress path keeps its exact upstream behavior.
    """
...
async def _resolve_media_tags_to_platform_assets(text: str) -> str:
    ...
    Platform-relay egress must not hand out pod-local paths (dead text for web users — no shared
    filesystem) and must not fall back to ``_resolve_media_to_data_urls`` (which would inline
    multi-MB base64 blobs into relay transcripts). Instead the file is uploaded to the platform
    asset library and the tag becomes ``asset:<uuid>``, which the web chat renders as a media
    card; non-uploadable tags degrade to the bare filename. Uploads are blocking network I/O (a
    video PUT can take minutes), so the resolution runs off the event loop.
    """
    # Case-insensitive like the shared IGNORECASE tag matchers — a hand-written ``media:`` tag must
    # not skip resolution here when the resolver would rewrite it. (_resolve_media_to_data_urls
    # above keeps its case-sensitive guard untouched: exact upstream parity.)
...
def _request_nolgia_token(body: Dict[str, Any]) -> tuple:
    ...
    The run-scoped Nolgia platform credential (turn-scoped attribution) is accepted on the
    platform-facing ``/v1/runs`` submission and on the pod-internal wake self-post to
    ``/v1/chat/completions``, which carries the ORIGINATING run's credential so a continuation
    attributes to the turn whose work it finishes. Validated like the session-key header
    (printable, no CR/LF/NUL, bounded) and NEVER logged; absent/empty means "no run scoping" and
    keeps the pod-wide ``NOLGIA_TOKEN`` behavior.
    """
    raw = body.get("nolgia_token") if isinstance(body, dict) else None
    if raw is None:
        return "", ""
...
# Body fields EXCLUDED from the POST /v1/runs Idempotency-Key fingerprint. ``nolgia_token`` is a
# short-lived per-attempt credential a supervisor may re-mint on retry, and a rotated token must
# still match the original submission — a fingerprint miss would 409 the retry (or, before the
# durable store, re-execute the run): exactly the double-billing this header exists to prevent.
# Everything else in the body (input, history, session_id, workspace_id, model, ...) is a stable
# identity and stays in the fingerprint.
_RUNS_IDEMPOTENCY_FINGERPRINT_EXCLUDED_KEYS = frozenset({"nolgia_token"})
...
class APIServerAdapter(OpenAICompatRoutesMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Per-instance boot identity for the restart journal (NOL-423). One adapter instance ==
        # one gateway boot serving /v1/runs: in-memory run state (streams, approval queues,
        # executor threads) dies with the instance, so journal rows carrying any OTHER boot_uuid
        # are, by construction, runs whose work this process cannot be executing.
        self._boot_uuid: str = uuid.uuid4().hex
        try:
            self._pod_identity: str = os.getenv("HOSTNAME") or socket.gethostname() or ""
        except Exception:
            self._pod_identity = ""
        # Journal heartbeat throttle state and the bounded partial-output tail flushed with
        # heartbeats (memory only between journal writes).
        self._run_journal_heartbeat_at: Dict[str, float] = {}
        self._run_output_tails: Dict[str, str] = {}
        # Effective state of the restart-journal contract (NOL-423), and the only thing
        # /v1/capabilities may advertise it from. False until connect() proves BOTH halves: the
        # store is on-disk durable (a :memory: fallback journal dies with the process) and
        # prior-boot reconciliation actually ran for this boot. A platform that stops treating a
        # post-restart 404 as the lost-run signal on the strength of this flag must never see it
        # set on a pod that cannot keep the promise.
        self._run_journal_ready: bool = False
        # Scoped-lock identity held while this boot owns the journal's store (set by
        # _claim_run_journal_ownership, released on disconnect).
        self._run_journal_lock_identity: Optional[str] = None
        # Run-scoped Nolgia platform tokens retained per session (NOL-413). A /v1/runs submission
        # binds its ``nolgia_token`` for the run's own turn, but pod-INTERNAL continuation turns of
        # the same session — gateway.wake's self-POST to /v1/chat/completions after a background
        # delegation completes — enter through _run_agent(). A wake carries the ORIGINATING run's
        # credential in its request body (see gateway.wake._self_post_chat_completion) and needs
        # nothing from here; this map is the fallback for continuations that carry none (a native
        # session-chat turn, a durably-replayed completion whose in-memory token is gone). Without
        # it their tool subprocesses fall back to the pod-wide NOLGIA_TOKEN and every ``nolgia
        # assets upload`` they make lands UNATTRIBUTED (no agent_session_id stamp).
        #
        # Keyed by (profile, session_id) — see _session_run_token_key: session ids are
        # caller-controlled and in multiplex mode every ``/p/<profile>/...`` route shares this
        # adapter, so the profile scope is part of the identity, never just the id. Each value is
        # the LIST of that key's live credentials, oldest first — one entry per submitted run,
        # ``(token, monotonic-deadline, live-holder-count)``. Holder counts belong to the run that
        # owns the token, never to the key: concurrent runs on one session settle in arbitrary
        # order, so a shared counter would keep an already-settled run's token "live" (and past
        # its server-side grace, expired) for as long as a sibling run executes. An entry cannot
        # expire while its own run still holds it, and its deadline is a settle grace applied when
        # that run releases (see _SESSION_RUN_TOKEN_SETTLE_GRACE_S). Tokens are copied into a
        # task-local ContextVar at bind time, so concurrent turns never share a mutable slot.
        self._session_run_tokens: Dict[tuple, List[tuple]] = {}
        self._session_run_tokens_lock = threading.Lock()
        # Push-based marketplace ability freshness (NOL-416): SSE subscriber + between-turns
        # installer + turn-boundary version check. Constructed in connect() only when the pod
        # holds platform credentials.
        self._ability_freshness: Optional[Any] = None
        self._ability_events_task: Optional["asyncio.Task"] = None
        # Confirmed-in-library media GC sweeper (NOL-516). Started in connect() only when the pod
        # holds platform credentials.
    ...
    # -- Ability freshness (NOL-416) / media GC (NOL-516) -------------------------------

    def _start_ability_freshness(self) -> None:
        """Construct the freshness manager and launch its SSE subscriber.

        Idempotent across reconnects (the old task is cancelled). Import is lazy and failures are
        soft: freshness plumbing must never take the API server down.
        """
        try:
            from gateway.ability_freshness import AbilityFreshnessManager, ability_freshness_enabled

            if not ability_freshness_enabled():
                return
            if self._ability_freshness is None:
                self._ability_freshness = AbilityFreshnessManager(idle_check=self.active_agent_work_count)
    ...
    def _start_media_gc(self) -> None:
        ...
        The safety net behind the post-upload deletion hook: media the agent wrote locally but
        never delivered through a MEDIA: tag (``nolgia gen --out`` downloads, intermediate renders)
        is reclaimed only after the platform library is proven to hold the same bytes. Idempotent
        across reconnects; lazy import and soft failures — GC must never take the API server down.
        """
        try:
    ...
    async def _ensure_abilities_fresh(self) -> None:
        """Turn-boundary check: the run about to execute must see the latest published version of
        every installed ability. Cheap no-op while the push subscription is live and caught up;
        fail-open otherwise."""
    ...
    def _notify_ability_turn_finished(self) -> None:
        """Turn-end hook: give deferred ability installs their between-turns window."""
        manager = self._ability_freshness
        if manager is not None:
            try:
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        # Derived from the EFFECTIVE configuration, never hardcoded: the platform raises per-user
        # concurrency on this flag, so a deployment whose session workspaces are off (mode off, an
        # operator-pinned terminal.cwd, a non-local terminal backend) or whose scratch base is not
        # writable must not advertise isolation it cannot provide (NOL-414).
        try:
        ...
        return web.json_response({
            ...
            "features": {
                ...
                # POST /v1/runs honors Idempotency-Key: replaying a key with an identical body
                # returns the ORIGINAL run_id without starting a second run. The Nolgia platform
                # relay gates widening its submit-retry classes (ambiguous 502/504, lost 202) on
                # this boolean (NOL-397) — pods roll gradually, so it must not retry broadly against
                # a pod that would double-execute. (``runs_idempotency`` above is upstream's richer
                # record of the same surface; both are advertised.)
                "run_idempotency": True,
                # Run submissions are journaled durably and, after a restart, prior-boot in-flight
                # runs are reported as failed with error_code="gateway_restart" (+ provenance)
                # instead of 404ing (NOL-423). EFFECTIVE state, not intent: false when
                # response_store.db could not be opened (the :memory: fallback journal dies with
                # the process) or when startup reconciliation did not run for this boot.
                "run_restart_journal": bool(self._run_journal_ready),
                # POST /v1/runs accepts a run-scoped Nolgia platform token ("nolgia_token") and
                # binds it to the run's session context: the run's platform egress (asset uploads)
                # and tool subprocesses (the nolgia CLI) then authenticate as the causing turn. The
                # platform relay keys per-user concurrent turn execution on this flag (NOL-207).
                "nolgia_run_token": True,
                # Runs execute inside a session-scoped scratch workspace (<base>/session-<id8>/)
                # enforced as the turn's default cwd/TMPDIR, and POST /v1/runs accepts an optional
                # "workspace_id" naming the workspace identity explicitly (NOL-414). Reports the
                # EFFECTIVE state (gateway.session_workspaces config + provisionable scratch base).
    ...
    def _bind_api_server_session(
        *, chat_id: str = "", session_key: str = "", session_id: str = "",
        ...
        nolgia_token: str = "", workspace_id: str = "", profile: str = "") -> list:
        ...
        in gateway.session_context): platform egress and tool subprocesses of THIS run then
        authenticate as the causing turn instead of the pod-wide bearer. Empty means "no run
        scoping" — everything falls back to pod env exactly as before.

        Per-session scratch workspace (NOL-414): every bind derives a session-scoped scratch dir
        (gateway.session_workspace, ``<base>/session-<id8>/``) from ``workspace_id`` (an explicit
        caller-supplied workspace identity, e.g. the Nolgia platform session UUID) falling back to
        the session identity, and enforces it as the turn's DEFAULT write surface: it becomes the
        terminal/file-tool cwd seed and the subprocess TMPDIR — so concurrent turns from different
        sessions stop sharing one scratch directory. The session's STABLE workspace (a
        surface-registered workspace pin for the durable conversation identity when one exists,
        else the seeded scratch workspace) is ALSO bound as the runtime cwd
        (``set_session_vars(cwd=...)``): that is what the system prompt's "Current working
        directory" line reports (``resolve_agent_cwd``); without it the prompt advertises the
        TERMINAL_CWD fallback — the shared home on the pod — and a model asked to work "in its
        current working directory" writes ABSOLUTE shared-home paths that bypass the seeded record
        entirely. The bind is deliberately NOT the mutable cwd record: `cd` state must stay out of
        the cached system prompt (advertising it would rebuild the whole prompt on every directory
        change). Context-file discovery is NOT re-anchored (``resolve_context_cwd`` ignores a
        session cwd equal to the bound scratch workspace) and absolute paths are not fenced.
        Deployments where workspaces do not apply bind exactly as before — no scratch dir, no
        runtime-cwd bind: ``gateway.session_workspaces.mode: off``, an operator-pinned
        ``terminal.cwd``, or a non-local terminal backend (``ensure_session_workspace`` returns None).

        ``profile`` is the multiplex profile serving THIS request (``_api_request_profile``, captured
        on the request task — the ContextVar does not follow ``run_in_executor`` threads, so callers
        must pass it explicitly). Bound as ``HERMES_SESSION_PROFILE`` so background work this turn
        spawns records the profile that owns it and its later wake self-posts go through
        ``/p/<profile>/...`` instead of the default-profile route (NOL-413).
        """
        from gateway.session_context import set_session_vars
        from gateway.session_workspace import ensure_session_workspace

        scratch_dir = ensure_session_workspace(workspace_id or session_id or chat_id or session_key)
        session_cwd = ""
        if scratch_dir:
            # Seed the per-session cwd RECORD (the terminal/file tools' first-rung cwd resolver) so
            # commands without an explicit workdir run inside the session workspace. Seed-only: a
            # continuing session that already has cwd state (the model `cd`'d somewhere) keeps it,
            # and a surface-registered cwd override (ACP/TUI workspace pinning) is never shadowed.
            # Both key spaces are seeded because the terminal resolves the record via the
            # approval/session key while the file tools use the raw task id (the run's session id).
            try:
                from tools.terminal_tool import get_session_cwd, record_session_cwd, resolve_task_overrides

                for key in {k for k in (session_key, session_id, chat_id) if k}:
                    if get_session_cwd(key) is None and not resolve_task_overrides(key).get("cwd"):
                        record_session_cwd(key, scratch_dir)
                # The turn's ADVERTISED working directory: DURABLE over per-run (``session_key`` on
                # /v1/runs is the run's approval key — a fresh run id every request — so it is read
                # LAST, or it would shadow a surface pin registered against the durable
                # conversation identity) and STABLE across turns (the mutable cwd RECORD is
                # deliberately not read: it changes whenever the model `cd`s, and advertising it
                # would flip the prompt's cwd line mid-conversation and rebuild the whole system
                # prompt — prompt-cache death). A surface-registered pin is stable by contract, so
                # it is what gets advertised when present; otherwise the session's scratch workspace.
                ...
                logger.debug("session workspace cwd seeding failed", exc_info=True)

        return set_session_vars(
            platform="api_server", chat_id=chat_id, session_key=session_key, session_id=session_id,
            browser_control_principal=browser_control_principal,
            browser_control_transport_family=browser_control_transport_family,
            cwd=session_cwd, scratch_dir=scratch_dir or "", async_delivery=False,
            nolgia_token=nolgia_token, profile=profile, cron_session="")

    # -- Retained run credentials per session (NOL-413) -----------------------------------

    # Grace applied to a retained run token AFTER its last live holder settles. While a holder is
    # live the entry never expires, so a run that outlives any fixed wall-clock window — a /v1/runs
    # foreground turn can legitimately run for hours — keeps its credential for its whole life. The
    # grace then sits INSIDE the credential's server-side validity: nolgia-api keeps a turn
    # credential valid while its turn is pending and for agentTurnCredentialSettleGrace (2h) after
    # it settles, so 90 minutes of grace can never present an already-expired credential. Past the
    # grace, continuation turns that carry no credential of their own fall back to the pod-wide
    # bearer (unattributed, never failing).
    _SESSION_RUN_TOKEN_SETTLE_GRACE_S = 90 * 60

    @staticmethod
    def _session_run_token_key(session_id: str, profile: str = "") -> tuple:
        """Retention identity of a session's run credential: ``(profile, session_id)``.

        The profile scope is PART of the key, never just the session id. In multiplex mode every
        ``/p/<profile>/...`` route shares this one adapter while each profile has an independent
        runtime, ``HERMES_HOME`` and secret scope; session ids are caller-controlled, so keying on
        the id alone would hand profile B's request the turn credential a profile A run retained
        under the same id (wrong account, wrong attribution). *profile* must be passed explicitly
        by callers that resolved it on the request task (``_api_request_profile`` does NOT follow
        ``run_in_executor`` threads). The default profile has TWO wire spellings — an unprefixed
        ``/v1/...`` request resolves ``""`` while ``/p/default/v1/...`` resolves ``"default"`` —
        and both canonicalize to ``""`` here.
        """
    ...
    def _prune_session_run_tokens_locked(self, now: float) -> None:
        ...
        for key, entries in list(self._session_run_tokens.items()):
            live = [entry for entry in entries if entry[2] > 0 or entry[1] > now]
            if live:
                self._session_run_tokens[key] = live
            else:
                del self._session_run_tokens[key]

    def _retain_session_run_token(self, session_id: str, nolgia_token: str, *, profile: str = "") -> None:
        """Retain *nolgia_token* as this (profile, session)'s run credential.

        Called by the /v1/runs submission path and paired with :meth:`_release_session_run_token`
        when that run settles: the run is counted as a live HOLDER of ITS OWN entry, and a held
        entry never expires. Retention is therefore bounded by the originating run's lifecycle
        plus a settle grace, not by a fixed window from request receipt, and a run's liveness never
        props up a sibling run's token. A newer run for the same (profile, session) is appended, so
        a continuation that carries no credential of its own attributes to the most recent live
        platform-submitted turn. Continuations that must attribute to a SPECIFIC originating run
        carry that run's credential explicitly (gateway.wake) instead of reading this map.
        """
        ...
        with self._session_run_tokens_lock:
            ...
            # A resubmission of the SAME credential (retry, duplicate run of one platform turn)
            # shares the turn's entry: one more holder on it, not a second copy that could be
            # released out from under the other.
    ...
    def _release_session_run_token(self, session_id: str, nolgia_token: str = "", *, profile: str = "") -> None:
        """Release the settling run's own hold on its retained credential.

        *nolgia_token* identifies the OWNER — the credential the settling run retained. Releasing
        by key alone would decrement whichever entry the key currently points at, so a short run
        settling first would hand its hold to a longer-running sibling's token. Starts the settle
        grace on that entry only: once its own run no longer holds it, it survives
        ``_SESSION_RUN_TOKEN_SETTLE_GRACE_S`` longer so late continuations still find it, then ages out.
        """
        if not session_id:
            return
        key = self._session_run_token_key(session_id, profile)
            ...
            if entries:
                for index, (token, _deadline, holders) in enumerate(entries):
                    if nolgia_token and token != nolgia_token:
                        continue
                    entries[index] = (token, now + self._SESSION_RUN_TOKEN_SETTLE_GRACE_S, max(0, holders - 1))
                    break
            self._prune_session_run_tokens_locked(now)
    ...
    def _retained_session_run_token(self, session_id: str, *, profile: str = "") -> str:
        ...
        The most recently retained STILL-LIVE entry wins: a run that already settled past its
        grace never shadows an older run that is still executing (its credential is the only one
        the platform still honors). Read-only: a continuation turn neither extends a grace nor
        becomes a holder — only a platform submission (which carries a fresh credential) does.
        """
        if not session_id:
            return ""
        key = self._session_run_token_key(session_id, profile)
    ...
    async def _run_agent(
        ...
        nolgia_token: str = "") -> tuple:
        """Create an agent and run one turn in a thread executor -> ``(result, usage)``.
        ``agent_ref[0]`` receives the agent so SSE writers can interrupt it; ``active_run_id``
        registers it in ``_active_run_agents``. Under a confirmed model lock the actual
        provider/model must match or the turn fails; ``runtime`` metadata is attached.
        ``nolgia_token`` is the ORIGINATING run's Nolgia credential when the caller knows it
        (gateway.wake's self-POST); absent one the session's retained run token is bound."""
        loop = asyncio.get_running_loop()
        # Turn boundary (NOL-416): every _run_agent() caller (chat completions, responses, native
        # session chat) is about to execute a turn — install any newer published versions of
        # installed abilities first so the turn never runs on stale content.
        ...
        # Continuation turns for a platform-submitted session arrive here with no request credential
        # of their own unless one was carried explicitly. Without a binding, every tool subprocess
        # of the continuation falls back to the pod-wide NOLGIA_TOKEN and its uploads land
        # unattributed (NOL-413). *nolgia_token* is the ORIGINATING run's credential when the caller
        # knows it — gateway.wake's self-POST carries the credential of the run whose detached
        # delegation it is finishing, so a wake attributes to THAT run even when the session has
        # since started newer turns. Absent one, fall back to the session's retained run token (see
        # _retain_session_run_token), keyed by the request's profile scope. Sessions never submitted
        # via /v1/runs resolve "" and keep today's behavior byte-for-byte.
        ...
        def _run():
            ...
            with self._profile_scope(request_profile):
                tokens = self._bind_api_server_session(
                    ...
                    nolgia_token=effective_nolgia_token, profile=str(request_profile or ""))
                agent = None
                try:
    ...
    # Journal heartbeat floor (seconds): non-transition journal writes (a tool lifecycle event
    # refreshing last_event) are throttled to at most one per interval per run. Status TRANSITIONS
    # always write. Message deltas never reach the journal directly — they only refresh the
    # in-memory output tail, flushed with the next heartbeat (NOL-423).
    _RUN_JOURNAL_HEARTBEAT_SECONDS = 15.0
    # Bounded partial-output tail journaled with heartbeats. With spot's 25s termination grace
    # (NOL-410) the journaled tail is often the only surviving record of a preempted run's progress.
    _RUN_JOURNAL_OUTPUT_TAIL_CHARS = 2000
    # Headroom on the in-memory accumulation buffer. Redaction is always applied to the WHOLE buffer
    # before any tail is sliced off it (a credential's recognizable prefix — "-----BEGIN … KEY-----",
    # "password=", "eyJ" — can sit before the retained boundary while the secret bytes extend into
    # it), so the buffer must hold enough leading context for the longest credential shapes
    # redact_sensitive_text knows.
    ...
    def _set_run_status(self, run_id: str, status: str, durable: bool = True, **fields: Any) -> Dict[str, Any]:
        # ``durable`` rides in ``fields`` only when set to False, so the delegate call keeps
        # upstream's exact (adapter, run_id, status, **fields) shape for the default case.
        if not durable:
            fields["durable"] = False
    ...
    def _claim_run_journal_ownership(self) -> bool:
        ...
        True only when no OTHER live gateway can be serving runs out of the same
        ``response_store.db``: this process holds no other connected adapter for that path
        (in-process registry) and the machine-local scoped lock for it is either free or held by a
        dead process (the lock's PID + start-time liveness check is the "prior boot is gone" proof).
        Fails closed: reconciliation is destructive-by-design for a live boot's runs (auto-resubmit
        duplicates side effects and credit spend), so any uncertainty means no reconciliation and
        ...
        with _JOURNAL_STORE_OWNERS_LOCK:
            ...
            if owner is not None and owner is not self and owner.is_connected:
                logger.warning(
                    "[%s] another adapter in this process is serving runs from %s; skipping "
                    "prior-boot run reconciliation (its in-flight runs are live, not lost)",
                    self.name, db_path)
                return False
        ...
        try:
            ...
            acquired, existing = acquire_scoped_lock(
                "api_server_run_journal", db_path,
                metadata={"boot_uuid": self._boot_uuid, "pod": self._pod_identity, "port": self._port})
        except Exception:
            logger.warning(
                "[%s] could not evaluate run-journal store ownership for %s; skipping prior-boot "
                "run reconciliation", self.name, db_path, exc_info=True)
            self._release_run_journal_ownership()
            return False
        if not acquired:
            logger.warning(
                "[%s] run journal store %s is held by a live gateway (%s); skipping prior-boot run "
                "reconciliation so its in-flight runs are not mis-reported as gateway_restart failures",
                self.name, db_path, (existing or {}).get("pid", "unknown"))
            self._release_run_journal_ownership()
            return False
    ...
    def _release_run_journal_ownership(self) -> None:
        ...
        if identity:
            try:
                ...
                logger.debug("[%s] failed to release run-journal store lock", self.name, exc_info=True)

    def _reconcile_prior_boot_runs(self) -> bool:
        """Fail every unsettled prior-boot journal row, once ownership is proven.

        Deliberately NOT done in ``__init__``: adapter construction happens on every reconnect
        attempt, including a contender that is about to lose the port with EADDRINUSE, and the
        original gateway's in-flight runs would then be durably marked ``gateway_restart`` (which
        the platform auto-resubmits) while they keep executing. Called from ``connect()`` after the
        listener is bound and store ownership is claimed, so the prior boot is provably gone. Runs
        synchronously with no ``await`` between the bind and here, so no request handler can
        observe the pre-reconciliation state. Returns True when reconciliation completed and the
        restart-journal contract is therefore live for this boot.
        """
        ...
        if reconciled:
            logger.warning(
                "[%s] marked %d prior-boot run(s) failed with error_code=gateway_restart: %s",
                self.name, len(reconciled), ", ".join(entry["run_id"] for entry in reconciled))
            # Keep upstream's idempotency records (the replay surface) in step with the durable
            # terminal outcome, so a replayed key answers the same failed status a poll does.
            for entry in reconciled:
                with suppress(Exception):
                    self._run_idempotency_store.update_status(entry["run_id"], entry)
        return True

    def _journal_run_progress(
        self, run_id: str, status: str, prior_status: Optional[str], last_event: Optional[str], now: float,
    ) -> None:
        """Journal a non-terminal status durably, at bounded frequency.

        Transitions (queued->running, running->waiting_for_approval, ...) write immediately —
        low-frequency and provenance-bearing. Repeat statuses (tool lifecycle events refreshing
        last_event) are heartbeats, throttled to _RUN_JOURNAL_HEARTBEAT_SECONDS apart. Best-effort:
        a store hiccup must never fail the run.
        """
        if status == prior_status and (
                now - self._run_journal_heartbeat_at.get(run_id, 0.0) < self._RUN_JOURNAL_HEARTBEAT_SECONDS):
            return
        ...
        if tail:
            # Same egress redaction the SSE stream applies before any text leaves the process —
            # applied to the whole buffer BEFORE the journaled tail is sliced, so a credential whose
            # prefix sits ahead of the boundary is still recognized.
            fields["output_tail"] = redact_sensitive_text(tail, force=True)[-self._RUN_JOURNAL_OUTPUT_TAIL_CHARS:]
        try:
            self._response_store.journal_run_update(run_id, status=status, **fields)
        except Exception:
            logger.debug("[api_server] failed to journal progress for run %s", run_id, exc_info=True)
        else:
    ...
    def _append_run_output_tail(self, run_id: str, delta: str) -> None:
        ...
        Trimming redacts first: dropping the head of raw text can strip the prefix (key header,
        ``password=``, JWT ``eyJ``) that makes a credential recognizable, leaving its remaining
        bytes to be journaled verbatim later.
        """
        tail = self._run_output_tails.get(run_id, "") + delta
        if len(tail) > self._RUN_JOURNAL_OUTPUT_RAW_CHARS:
            tail = redact_sensitive_text(tail, force=True)[-self._RUN_JOURNAL_OUTPUT_RAW_CHARS:]
    ...
    def _flush_run_output_heartbeat(self, run_id: str) -> None:
        ...
        A text-only response produces message deltas and nothing else: after the ``running``
        transition there is no further status event, so without this the journal would hold none
        of the partial output the restart contract promises survives a preemption. Routed through
        the same throttled heartbeat as tool lifecycle events — at most one journal write per
        ``_RUN_JOURNAL_HEARTBEAT_SECONDS`` per run, never one per delta. Called from the run's
        executor thread.
        """
        ...
        if not status or status in _api_runs.TERMINAL_STATUSES:
            return
        # prior_status == status keeps this on the throttled heartbeat path.
        self._journal_run_progress(run_id, status, status, current.get("last_event"), time.time())
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Push-based ability freshness (NOL-416): subscribe to the platform's ability-publish
            # SSE stream so marketplace content updates install between turns without a pod roll —
            # plus the turn-boundary installed-vs-latest check wired into the run entrypoints.
            # No-op unless the pod holds platform credentials.
            self._start_ability_freshness()
            # Confirmed-in-library media GC (NOL-516): reclaim local copies of media the platform
            # library provably already holds. No-op unless the pod holds platform credentials.
            ...
            try:
                ...
                return False
            # Startup reconciliation (NOL-423) — deliberately here, after the listener is ours.
            # Owning the port (plus the store lease claimed inside) is the proof that the boot whose
            # unsettled journal rows we are about to fail is actually gone: a contender that starts
            # against a live gateway's HERMES_HOME never reaches this line (EADDRINUSE above) or
            # fails the store-ownership check. Every unsettled prior-boot row becomes a durable
            # `failed` status with error_code="gateway_restart" + provenance, so the replacement pod
            # answers status polls honestly instead of 404ing and the platform's capability-gated
            # auto-resubmit (NOL-397 part 1) can act on a stable signal. Synchronous on purpose —
            # no await between the bind and here, so no request is served mid-reconciliation.
            self._run_journal_ready = self._reconcile_prior_boot_runs()
            if not self._run_journal_ready:
                logger.warning(
                    "[%s] restart-run-journal support is INACTIVE for this boot (no durable store or "
                    "reconciliation unavailable); /v1/capabilities reports run_restart_journal=false "
                    "so the platform keeps its pre-journal lost-run handling", self.name)
    ...
    async def disconnect(self) -> None:
        ...
        # Stop the ability freshness subscriber (NOL-416): the reconnect loop constructs a fresh
        # adapter, which starts its own. getattr-guarded: some tests exercise disconnect on
        # partially-constructed adapters.
        ...
        # Hand the journal store back before closing it: the next boot (or a replacement adapter in
        # this process) must be able to prove this one is gone in order to reconcile (NOL-423).
        self._run_journal_ready = False
        try:
            self._release_run_journal_ownership()
        except Exception:
            logger.debug("Failed to release run-journal ownership for %s", self.name, exc_info=True)
