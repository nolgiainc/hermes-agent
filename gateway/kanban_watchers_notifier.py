    ...
    * non-push or wake-only: the wake IS the delivery → it runs FIRST and the
    ...
    async def wake(self) -> None:
        ...
        if not self.is_push_adapter:
            # Same profile the subscription recorded: the self-post must re-enter through that
            # profile's /p/<profile>/ route, whose runtime owns the session (NOL-413).
            await deliver_wake(
                self.adapter, text=self.synth, session_id=self.session_key,
                profile=str(self.sub_profile or ""))
            self._log_woke()
            return
