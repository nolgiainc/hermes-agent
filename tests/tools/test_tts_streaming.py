...
def _enable_audio_paths(monkeypatch):
    """Opt this module out of the suite-wide audio kill-switch.

    tests/conftest.py sets HERMES_DISABLE_AUDIO_PLAYBACK=1 so the suite never
    plays sound on a developer machine. Every test in this file exercises the
    speaker dispatch path with sounddevice/voice_mode fully mocked, so no real
    audio can result — but with the kill-switch active stream_tts_to_speaker
    short-circuits to display-only and none of the playback assertions would
    ever be reached.
    """
...
def _timed_sync_run(monkeypatch, sentences, *, synth_s=0.12, play_s=0.12,
    ...
    fake_vm = MagicMock()
    # A bare MagicMock attribute would return a truthy Mock from
    # audio_playback_disabled() and silently suppress the speaker path.
