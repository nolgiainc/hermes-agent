...
def stream_tts_to_speaker(
    ...
    *tts_done_event* is **set** in ``finally`` so continuous voice mode knows playback finished."""
    ...
    try:
        ...
        # HERMES_DISABLE_AUDIO_PLAYBACK=1 → display-only mode: no streamer, no
        # sync pipeline, no synthesis, no speaker (the test suite sets it so a
        # run never plays sound on the developer's machine).
