...
def audio_playback_disabled() -> bool:
    ...
    ``tools.tts_tool_speaker``). Synthesis-to-file (``text_to_speech_tool``
    with an output path) is unaffected — only audible playback is suppressed.
    The test suite sets this in ``tests/conftest.py`` so running tests never
