...
def _vision_llm_settings() -> tuple[float, float]:
    """``auxiliary.vision`` ``(timeout, temperature)``. The timeout resolves through
    ``tools.vision_tools._resolve_vision_timeout`` (config / ``HERMES_VISION_TIMEOUT``,
    fork default 60s — a stalled vision call must not burn minutes); temperature
    defaults to 0.1 on any config error."""
    from tools.vision_tools import _resolve_vision_timeout

    timeout = _resolve_vision_timeout()
    try:
        cfg = cfg_get(load_config(), "auxiliary", "vision", default={})
        return timeout, float(cfg.get("temperature", 0.1))
    except Exception:
        return timeout, 0.1
