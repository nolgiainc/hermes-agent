...
def stream_tts_to_speaker(
    ...
    try:
        ...
        # finally block, which flushes anything still queued). Skipped entirely
        # when audio playback is disabled (display-only mode).
        sync_pipeline = (
            _SyncSentencePipeline(stop_event)
            if (streamer is None and not playback_disabled)
            else None
        )
