...
| `HERMES_VISION_TIMEOUT` | Per-attempt timeout in seconds for the vision/video LLM call itself (override for `auxiliary.vision.timeout`; default: `60`). Distinct from `HERMES_VISION_DOWNLOAD_TIMEOUT`, which covers only the HTTP image fetch. `video_analyze` floors the resolved value at 180s. |
| `HERMES_VISION_PROBE_TIMEOUT` | Reduced per-attempt budget in seconds used by `vision_analyze` once the consecutive-timeout guard has ordered a strategy switch (override for `auxiliary.vision.probe_timeout`; default: `15`). Keeps a degraded vision route from burning full budgets on every call while still discovering recovery — any success restores the full budget. Values `<= 0` disable the cap. |
| `HERMES_VISION_MAX_DIMENSION` | Longest image side in pixels sent to the vision LLM by `vision_analyze` (override for `auxiliary.vision.max_dimension`; default: `2048`). Larger images (e.g. contact sheets and other dense composites) are downscaled before the call — cloud vision routes downscale internally to about this size anyway, so oversized sends only add upload and tiling latency. Values `<= 0` disable the cap. |
...
| `AUXILIARY_VIDEO_MODEL` | Override model for the `video_analyze` tool (falls back to `AUXILIARY_VISION_MODEL`). Legacy env form of `auxiliary.video.model`; `auxiliary.video.{provider, fps, detail}` are config-only. Use a Gemini model (e.g. `gemini-3.8-flash`) — it is the only route that attaches the clip through an OpenAI-compatible proxy. With Gemini, YouTube URLs are fetched by Google; every other URL is downloaded by the tool and inlined (like a local file) up to ~14 MB of source video. |
