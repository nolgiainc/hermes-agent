...
def _get_bedrock_runtime_client(region: str, timeout: Optional[float] = None):
    ...
    ``timeout`` is the caller's per-attempt budget in seconds (NOL-135). botocore
    has no per-request timeout — connect/read timeouts are fixed at client
    construction — so a budgeted call gets its own client, cached under
    ``region#t<seconds>``. Distinct budgets come from per-task auxiliary config
    (a handful of values), so the cache stays small.
    ...
    if cache_key not in _bedrock_runtime_client_cache:
        ...
        _bedrock_runtime_client_cache[cache_key] = _require_boto3().client(
...
def invalidate_runtime_client(region: str) -> bool:
    """Evict one region's cached ``bedrock-runtime`` clients (stale HTTP pool); True if any evicted.

    Evicts the bare-region entry AND every budgeted ``region#t<seconds>`` variant — a stale
    connection pool is a property of the region's endpoint, not of one timeout budget.
    """
    prefix = f"{region}{_RUNTIME_CACHE_TIMEOUT_SEP}"
    doomed = [key for key in _bedrock_runtime_client_cache if key == region or key.startswith(prefix)]
    for key in doomed:
...
def build_converse_kwargs(
    ...
    reasoning_config: Optional[Dict] = None, additional_model_request_fields: Optional[Dict] = None,
) -> Dict[str, Any]:
    ...
    ``reasoning_config`` is the Hermes reasoning dict ({"enabled": ..., "effort": ...}); it is
    translated into native thinking fields via :func:`build_reasoning_request_fields`.
    ``additional_model_request_fields`` is a raw passthrough for caller-supplied model-native body
    fields (chat.completions ``extra_body``); the translated reasoning fields win over a raw
    duplicate (NOL-139)."""
    ...
    # Model-native body fields (NOL-139). ``additionalModelRequestFields`` is Converse's raw
    # passthrough into the model invocation body — the Converse counterpart of chat.completions
    # ``extra_body``. Assemble caller fields first, then let the translated reasoning fields win
    # over any raw duplicate so an explicit reasoning_config is authoritative.
    additional_fields: Dict[str, Any] = {}
    if additional_model_request_fields and isinstance(additional_model_request_fields, dict):
    ...
    if reasoning_fields:
        ...
        if thinking.get("type") == "enabled":
            # Manual (budget_tokens) thinking: Anthropic requires max_tokens > budget_tokens —
            # mirror build_anthropic_kwargs' bump — and rejects non-default sampling params
            # alongside thinking, so drop them rather than 400 the call.
            budget = int(thinking.get("budget_tokens") or 0)
            inference_config["maxTokens"] = max(max_tokens or 0, budget + 4096)
...
def call_converse(
    ...
    stop_sequences: Optional[List[str]] = None, guardrail_config: Optional[Dict] = None,
    timeout: Optional[float] = None, reasoning_config: Optional[Dict] = None,
    ...
    ``timeout`` is the caller's per-attempt budget in seconds (NOL-135). None keeps botocore's
    defaults (60s read timeout plus its own retries); a budget routes through a client configured
    to honour it — see :func:`_get_bedrock_runtime_client`. ``reasoning_config`` /
    ``additional_model_request_fields`` are forwarded to :func:`build_converse_kwargs` (NOL-139)."""
    ...
    kwargs = build_converse_kwargs(
        ...
        reasoning_config=reasoning_config, additional_model_request_fields=additional_model_request_fields,
    )
    try:
