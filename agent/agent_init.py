...
_TURN_STATE: Dict[str, Any] = {
    ...
    # Rate-limits durable SessionDB stamps. -inf ("never persisted"), not 0.0: the due-check
    # subtracts this from time.monotonic(), which counts from boot — on a freshly-booted pod
    # with uptime under the heartbeat interval, 0.0 reads as "just persisted" and the
    # session's first durable stamp is silently swallowed.
    "_session_activity_last_persist_mono": float("-inf"),
    "_current_tool": None,
    "_api_call_count": 0,
