...
def is_gemini_flash_38_or_later(model: str) -> bool:
    ...
    Starting with gemini-3.8-flash (GA 2026-09-02) the Flash line's request contract tightened: the
    model rejects ``thinkingLevel: "minimal"`` (only low/medium/high are accepted) and Google's
    guidance is to strip ``temperature`` / ``topP`` / ``topK`` from requests entirely. Earlier Flash
    generations (3.7 and below, 2.5) still accept all of those, so this is a version comparison on
    the model id — not a prefix match. True for 3.8, 3.9 and any 4.x Flash (including ``-lite`` /
    ``-preview`` variants); false for 3.7 and earlier and for every Pro model. Accepts ``google/`` /
...
def _file_data_part(item: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """Video part for an OpenAI-style ``file`` item (video_analyze): a ``data:`` URL becomes
    ``inlineData``, an ``https://`` URL a ``fileData`` reference (Google fetches YouTube itself);
    ``video_metadata`` / ``detail`` map to ``videoMetadata`` / ``mediaResolution``. None otherwise."""
    if item.get("type") != "file":
        return None
    file_part = item.get("file") or {}
    file_data, mime = file_part.get("file_data"), file_part.get("format")
    ...
    if file_data.startswith("data:"):
        try:
            ...
            return None
        part: Dict[str, Any] = {"inlineData": {"mimeType": mime, "data": encoded}}
    ...
    if isinstance(detail, str) and detail:
        part["mediaResolution"] = {"level": f"MEDIA_RESOLUTION_{detail.strip().upper()}"}
    return part
...
def _multimodal_part(item: Any) -> Optional[Dict[str, Any]]:
    ...
    if text or isinstance(item, str):
        return {"text": text}
    return (_inline_data_part(item) or _file_data_part(item)) if isinstance(item, dict) else None
...
def build_gemini_request(
    *, messages: List[Dict[str, Any]], tools: Any = None, tool_choice: Any = None, temperature: Optional[float] = None,
    ...
    # gemini-3.8-flash and later Flash generations reject sampling overrides and the "minimal"
    # thinking level (see is_gemini_flash_38_or_later): drop temperature/topP, clamp minimal -> low.
    strict_flash = is_gemini_flash_38_or_later(model)
    if strict_flash and (temperature is not None or top_p is not None):
        logger.debug("%s rejects sampling overrides; dropping temperature/topP from generationConfig", model)
        temperature = top_p = None
    thinking = _normalize_thinking_config(thinking_config)
    if strict_flash and thinking and thinking.get("thinkingLevel") == "minimal":
