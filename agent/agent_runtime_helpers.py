...
def repair_empty_non_final_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for idx, msg in enumerate(messages):
        ...
        # Assistant turns ONLY: an empty USER message is never legitimate to send (NOL-216), so
        # ``_drop_empty_turns_and_merge_users`` later in ``sanitize_api_messages`` removes it
        # outright and merges any ``user, user`` seam the drop leaves behind. Substituting a
        # placeholder here would make that pass see non-empty content and ship a user turn the
        # user never wrote.
        if idx != last_idx and isinstance(msg, dict) and msg.get("role") == "assistant" and not _msg_has_payload(msg):
...
def _drop_empty_turns_and_merge_users(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """FINAL GUARD — never emit an assistant or user message with no payload.

    After the dedupe pass, an assistant whose tool_calls were ALL filtered as duplicates (a
    colliding-id turn — NOL-106) is left as ``{"role":"assistant","content":""}``. Moonshot/OpenAI
    reject an assistant message with neither content nor tool_calls with a NON-RETRYABLE HTTP 400
    ("the message at position N with role 'assistant' must not be empty"), which permanently
    wedges the session — every retry resends the identical poisoned history. Drop any assistant
    that carries NO payload of any kind (no content, no tool_calls, no reasoning), then sweep the
    ``tool`` results that depended on it (their calls were the duplicates; the surviving answers
    live under the earlier id-holder). Running on the per-call copy at the final chokepoint means
    this ALSO repairs PRE-EXISTING poisoned histories written by the old code — a wedged session
    un-sticks on its next turn with no manual DB surgery. Reasoning-only assistants are
    preserved: strict thinking providers (Kimi/DeepSeek) require the reasoning_content echo, and
    those turns are handled by _is_thinking_only_assistant, not here.

    NOL-216: the identical permanent-wedge shape occurs on the USER side. An empty-content user
    message (content ``""`` / whitespace-only / an empty list) reaches here from an interrupted
    or aborted turn, a host-fed history, or a bare re-send. Moonshot rejects it with the SAME
    non-retryable HTTP 400 ("the message at position N with role 'user' must not be empty"),
    which re-poisons every subsequent turn exactly like the assistant case. An empty user message
    is never legitimate to send, so drop it in this same pass — the adjacent-user merge below then
    repairs any ``user, user`` seam the drop leaves behind. Multimodal user turns whose text is
    empty but which carry image/other content parts are NOT empty and are kept.
    """
    kept: List[Dict[str, Any]] = []
    dropped_empty_assistants = 0
    dropped_empty_users = 0
    for msg in messages:
        if isinstance(msg, dict) and msg.get("role") == "assistant" and not _assistant_has_payload(msg):
            dropped_empty_assistants += 1
            continue
        if isinstance(msg, dict) and msg.get("role") == "user" and not _user_has_content(msg):
            dropped_empty_users += 1
            continue
        kept.append(msg)
    if not (dropped_empty_assistants or dropped_empty_users):
        return messages
    ...
    for msg in kept:
        ...
        swept.append(msg)
    # Dropping an empty assistant that sat between two user turns (``user → empty assistant →
    # user`` in a host-fed/legacy history) leaves two consecutive ``user`` messages, which strict
    # providers reject. The downstream ``drop_thinking_only_and_merge_users`` pass only merges
    # when it ITSELF dropped a thinking-only turn, so it will not repair this — merge the
    # newly-adjacent user messages here (string content joined with a blank line;
    # multimodal/list content is left as separate turns).
    merged: List[Dict[str, Any]] = []
    for msg in swept:
        prev = merged[-1] if merged else None
        if (
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    messages = _drop_empty_turns_and_merge_users(messages)
