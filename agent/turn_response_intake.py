...
def _namespace_provider_tool_call_ids(agent: Any, assistant_message: Any, messages: Any) -> None:
    """NOL-106: de-collide provider-minted tool_call ids BEFORE anything consumes them (tool
    dispatch, result pairing, persistence, replay). Moonshot/Kimi mint ids from a
    per-conversation counter that resets/overlaps across turns; stored verbatim they collide at
    assembly, get deduped to an empty assistant, and 400 the session permanently. Only ids that
    actually collide with an id already in the in-context history (or an earlier call in this
    response) are rewritten, so unique-id providers (incl. strict-format ones like
    Mistral/Anthropic) are untouched. Rewriting the single normalized object here keeps
    within-turn call↔result pairing intact."""
    ...
    for hm in messages:
        ...
        if hm.get("role") == "assistant":
            for htc in hm.get("tool_calls") or []:
                hcid = agent._get_tool_call_id_static(htc)
                if hcid:
                    existing_ids.add(hcid)
        elif hm.get("role") == "tool":
            hcid = (hm.get("tool_call_id") or "").strip()
            if hcid:
                existing_ids.add(hcid)
    rewritten = agent._namespace_tool_call_ids(assistant_message, existing_ids)
    if rewritten:
        logger.debug(
            "%sDe-collided %d provider tool_call id(s) for uniqueness", agent.log_prefix, rewritten,
        )
...
def normalize_model_response(
    ...
    _namespace_provider_tool_call_ids(agent, assistant_message, messages)
