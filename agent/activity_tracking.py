...
class ActivityTrackingMixin:
    ...
    def _persist_session_activity_if_due(self) -> None:
        ...
        # -inf default, not 0.0: monotonic() counts from boot, so on a freshly-booted host with
        # uptime below the heartbeat interval a 0.0 "never persisted" sentinel reads as "just
        # persisted" and swallows the session's first durable stamp. -inf is always due.
        last_mono = getattr(self, "_session_activity_last_persist_mono", float("-inf"))
