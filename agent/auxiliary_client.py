...
# The Anthropic adapter additionally translates a top-level ``response_format``
# into ``output_config.format`` (see _translate_anthropic_response_format); the
# Bedrock shim does not, so that key is only "handled" on the Anthropic route.
_ANTHROPIC_ADAPTER_HANDLED_KWARGS = _SHIM_ADAPTER_HANDLED_KWARGS | frozenset({"response_format"})
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        # Say — once per kwarg — when a caller passes something this adapter has no
        # translation for, instead of discarding it in silence (NOL-139).
        _warn_unknown_kwargs("AnthropicAuxiliaryClient", kwargs, _ANTHROPIC_ADAPTER_HANDLED_KWARGS)
        model = kwargs.get("model", self._model)
        ...
        # ``stop`` and ``top_p`` are natively supported by the Messages API (as stop_sequences /
        # top_p) — forward them instead of dropping (NOL-139). top_p follows the same model guard
        # as temperature, plus one extra: with extended thinking active, Anthropic only accepts
        # top_p in [0.95, 1], so an arbitrary caller value would 400 the call — report that drop
        # rather than silence it.
        stop = kwargs.get("stop")
        if stop:
            anthropic_kwargs["stop_sequences"] = [stop] if isinstance(stop, str) else list(stop)
        ...
        # Per-request HTTP headers (OpenCode session affinity, Copilot ``x-initiator``, ...) —
        # the Anthropic SDK accepts ``extra_headers`` on messages.create/stream too (NOL-139).
        # Caller headers win per key over whatever build_anthropic_kwargs produced, EXCEPT
        # ``anthropic-beta``, where a blanket override would silently disable the adapter's
        # feature betas (fast mode, OAuth) — combine the two beta lists instead.
        ...
        # Preserve the chat.completions timeout contract (NOL-135). Every auxiliary task computes
        # a per-attempt budget and passes it here; the Anthropic SDK takes it as a per-request
        # option on both messages.stream() and messages.create(), so it costs one key to honour.
        # Dropping it fell back to the client-level default — build_anthropic_client() uses 900s
        # when no timeout is supplied, and every auxiliary Anthropic client is built without one —
        # so a hung Anthropic-compatible gateway stalled a 30s compression or 60s vision call for
        # fifteen minutes with nothing in the logs to explain it.
        timeout = kwargs.get("timeout")
        if timeout is not None:
...
class _BedrockCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        # Say — once per kwarg — when a caller passes something this adapter has no
        # translation for, instead of discarding it in silence (NOL-139).
        _warn_unknown_kwargs("BedrockAuxiliaryClient", kwargs, _SHIM_ADAPTER_HANDLED_KWARGS)
        model = kwargs.get("model", self._model)
        max_tokens = kwargs.get("max_tokens") or kwargs.get("max_completion_tokens")
        ...
        if kwargs.get("tool_choice") is not None:
            # Converse toolChoice isn't wired through call_converse(); no in-tree auxiliary
            # caller passes tool_choice today. A DEBUG line is invisible in production, so in
            # practice that was still a silent drop — report it at WARNING, deduped (NOL-139).
            _warn_kwarg_unforwarded(
        ...
        if kwargs.get("extra_headers"):
            # botocore signs requests (SigV4) and pins headers at client construction; injecting
            # per-call headers would need a botocore event hook registered on a shared, cached
            # client — racy across concurrent auxiliary calls with different headers. Report the
        ...
        # Reasoning + model-native body fields (NOL-139). Reasoning priority mirrors the Anthropic
        # adapter: an explicit per-call config (``_reasoning_config``, set by _build_call_kwargs)
        # wins over an ``extra_body.reasoning`` dict (auxiliary.<task>.extra_body config / the
        # generic profile fallback — the shape provider="bedrock" actually produces). Remaining
        # extra_body keys ride Converse's native ``additionalModelRequestFields`` passthrough;
        # ``_``-prefixed keys are private Hermes plumbing, never wire fields.
        reasoning_cfg = kwargs.get("_reasoning_config")
        ...
        # Preserve the chat.completions timeout contract (NOL-135). botocore has no per-request
        # timeout, so call_converse() resolves a client configured for this budget rather than
        # the 60s botocore default multiplied by botocore's own read-timeout retries.
