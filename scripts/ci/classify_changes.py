...
def _py_irrelevant(p: str) -> bool:
    # Python sources are python-relevant wherever they live, including inside the otherwise-prose
    # trees in _PY_SKIP: the skill-docs freshness test runs website/scripts/generate-skill-docs.py
    # for real, so a generator-only edit changes what that test expects.
    if p.endswith(".py") or p.startswith(_PY_RELEVANT_SITE):
        return False
    return (
        _is_docs(p)
        or p in _ROOT_NPM
