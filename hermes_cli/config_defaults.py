...
DEFAULT_CONFIG = {
    ...
    "auxiliary": {
        ...
        # Vision budgets (NOL-197/NOL-253): timeout is PER ATTEMPT (env HERMES_VISION_TIMEOUT);
        # probe_timeout = reduced per-attempt budget while the consecutive-timeout guard is active
        # (env HERMES_VISION_PROBE_TIMEOUT; <= 0 disables); max_dimension = longest side in px sent
        # to the vision LLM, larger images are downscaled first (env HERMES_VISION_MAX_DIMENSION;
        # <= 0 disables).
        "vision": _aux(60, probe_timeout=15, max_dimension=2048, download_timeout=30),
        # video_analyze route: provider/model "" = inherit auxiliary.vision; fps = Gemini frame
        # sampling (0/null = provider default); detail = Gemini 3+ media resolution.
        "video": {"provider": "", "model": "", "fps": 1, "detail": ""},
    ...
    "gateway": {  # Gateway settings (messaging platforms: Telegram, Discord, Slack, ...).
        # Per-session scratch workspaces (NOL-414). On a host serving many concurrent sessions (the
        # API server), every turn used to share one default cwd and one TMPDIR, so two runs could
        # overwrite each other's fixed-path files. When engaged, each session's turns get
        # <scratch_base>/session-<id8>/ as their default cwd + subprocess TMPDIR.
        #   mode: "auto" engages unless terminal.cwd pins a real project workspace (an explicit
        #     "all my turns work here" choice); "on" forces it, "off" disables it. Only the local
        #     terminal backend is affected — a container/SSH backend executes where a host
        #     workspace path does not exist.
        #   scratch_base: base directory for the per-session dirs; empty means $HERMES_HOME/tmp.
        #   retention_hours: how long an idle workspace survives before a throttled sweep deletes
        #     it (stateless requests mint one per call, and $HERMES_HOME/tmp usually has no tmp
        #     cleaner). 0 disables the sweep.
        "session_workspaces": {"mode": "auto", "scratch_base": "", "retention_hours": 48},
